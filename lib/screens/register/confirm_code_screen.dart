...
class Otp extends StatefulWidget {
  ...
  const Otp({Key? key, required this.email, required this.id})
      : super(key: key);
  ...
  get _getResendButton {
    return InkWell(
      ...
      onTap: () {
        ...
        if (!sendingotpagain) {
          sendingotpagain = true;
          setState(() {});
          HttpClient().sendotpemail(widget.id.toString()).then((value) {
            sendingotpagain = false;
            setState(() {});
            showSuccessFlushBar(
                context: context,
  ...
  void _setCurrentDigit(int i) {
    setState(() {
      ...
      if (_firstDigit == -1) {
        ...
        Map<String, dynamic> body = {'otp_email': otp};

        HttpClient().varifyotp(widget.id.toString(), body).then((value) {
          Navigator.pop(context);
          showvarifiedorRejectedDialogue(varified: true);
        }).catchError((error) {
